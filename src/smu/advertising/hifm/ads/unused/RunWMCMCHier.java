...
public class RunWMCMCHier extends BaseRun {
	public static void main(String[] args) {
		...
		for (String type : types) {
			...
			try {
				BufferedWriter out = Files.newBufferedWriter(Paths.get("../data/WMCMC_" + loss + "_" + type + "_hybrid.out"), Charset.forName("UTF-8"));
				for (int minExpose : minExposes) {
					for (int i = 1; i <= numFolds; i++) {			// no hierarchy
						...
						fm.learn(train, test);
						BufferedWriter log = Files.newBufferedWriter(Paths.get(baseFile + "_WMCMC_" + loss + "_" + type + "_hybrid.log"), Charset.forName("UTF-8"));
