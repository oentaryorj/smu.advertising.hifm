...
import larc.recommender.learn.MCMC;
...
public class RunMCMCHier extends BaseRun {
	public static void main(String[] args) {
		...
		for (String type : types) {
			...
			try {
				BufferedWriter out = Files.newBufferedWriter(Paths.get("../data/MCMC_" + loss + "_" + type + "_hybrid.out"), Charset.forName("UTF-8"));
				for (int minExpose : minExposes) {
					for (int i = 1; i <= numFolds; i++) {			// no hierarchy
						...
						MCMC fm = new MCMC(Math.max(train.getNumCols(), test.getNumCols()), params, loss, reg);
						fm.learn(train, test);
						BufferedWriter log = Files.newBufferedWriter(Paths.get(baseFile + "_MCMC_" + loss + "_" + type + "_hybrid.log"), Charset.forName("UTF-8"));
