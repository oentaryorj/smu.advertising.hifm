...
public class RunWSGD extends BaseRun {
	public static void main(String[] args) {
		...
		for (String type : types) {
			for (Loss loss : losses) {
				try {
					BufferedWriter out = Files.newBufferedWriter(Paths.get("../data/WSGD_" + loss + "_" + type + ".out"), Charset.forName("UTF-8"));
					for (int minExpose : minExposes) {
						for (int i = 1; i <= numFolds; i++) {			// no hierarchy
							...
							fm.learn(train, test);
							BufferedWriter log = Files.newBufferedWriter(Paths.get(baseFile + "_WSGD_" + loss + "_" + type + ".log"), Charset.forName("UTF-8"));
