...
/**
 * A <code>ScoredPrecisionRecall</code> provides an evaluation
 ...
 *
 *
 * @author  Richard Oentaryo
 * @version 1.0
 */
public class ScoredPrecRecall {
    private final Set<Case> mCases = new TreeSet<Case>(Case.REVERSE_SCORE_COMPARATOR);
    private int mNegativeRef = 0;
    private int mPositiveRef = 0;
    private static final double[][] EMPTY_DOUBLE_2D_ARRAY = new double[0][];
    /**
     * Construct a scored precision-recall evaluation.
     */
    public ScoredPrecRecall() {
        /* do nothing */
    }
    /**
     * Clear all cases
    ...
    public double[][] prCurve(boolean interpolate) {
        PrecRecall eval = new PrecRecall();
    ...
    public double maximumFMeasure(double beta) {
        ...
        for (int i = 0; i < pr.length; ++i) {
            double f = PrecRecall.fMeasure(beta,pr[i][0],pr[i][1]);
    ...
    public double precisionAt(int rank) {
        ...
        int correctCount = 0;
        Iterator<Case> it = mCases.iterator();
    ...
    private static double[][] interpolate(List<double[]> prList, boolean interpolate) {
        ...
        return resultList.<double[]>toArray(EMPTY_DOUBLE_2D_ARRAY);
    }
