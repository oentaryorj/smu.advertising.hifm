...
import larc.recommender.learn.CD;
...
public class RunCDHier extends BaseRun {
	public static void main(String[] args) {
		...
		for (String type : types) {
			...
			try {
				BufferedWriter out = Files.newBufferedWriter(Paths.get("../data/CD_" + loss + "_" + type + "_hybrid.out"), Charset.forName("UTF-8"));
				for (int minExpose : minExposes) {			// agglomerative fitting + parent regularization
					for (int i = 1; i <= numFolds; i++) {			// no hierarchy
						...
						CD fm = new CD(Math.max(train.getNumCols(), test.getNumCols()), params, loss, reg);
						fm.learn(train, test);
						BufferedWriter log = Files.newBufferedWriter(Paths.get(baseFile + "_CD_" + loss + "_" + type + "_hybrid.log"), Charset.forName("UTF-8"));
