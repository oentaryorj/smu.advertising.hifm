...
import larc.recommender.learn.WSGD;
...
public class RunWSGDHier extends BaseRun {
	public static void main(String[] args) {
		...
		for (String type : types) {
			for (Loss loss : losses) {
				try {
					BufferedWriter out = Files.newBufferedWriter(Paths.get("../data/WSGD_" + loss + "_" + type + "_hybrid.out"), Charset.forName("UTF-8"));
					for (int minExpose : minExposes) {
						for (int i = 1; i <= numFolds; i++) {		// agglomerative fitting + parent (only) regularization
							...
							WSGD fm = new WSGD(Math.max(train.getNumCols(), test.getNumCols()), params, loss, reg);
							fm.learn(train, test);
							BufferedWriter log = Files.newBufferedWriter(Paths.get(baseFile + "_WSGD_" + loss + "_" + type + "_hybrid.log"), Charset.forName("UTF-8"));
